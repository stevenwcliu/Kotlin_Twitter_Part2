...
class TwitterClient(context: Context) : OAuthBaseClient(
    ...
    companion object {
        ...
        const val REST_CONSUMER_KEY =
            BuildConfig.CONSUMER_KEY // Change this inside apikey.properties: done

        const val REST_CONSUMER_SECRET =
            BuildConfig.CONSUMER_SECRET // Change this inside apikey.properties: done
    ...
    // using getApiUrl to get the full URL from the relative fragment and RequestParams to control the request parameters
    ...
    // send post requests (or put or delete)
    // These endpoint methods will automatically execute asynchronous requests signed with the authenticated access token.
